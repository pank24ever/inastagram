...
get '/follow/:user_id' do
  check_login
  connection.exec('insert into follow(following_id,followed_id) values($1,$2)',[session[:user?id],params['user_id']])
...
get '/unfollow/:user_id' do
  check_login
  connection.exec('delete from follow where following_id = $1 and followed_id = $2',[session[:id],params['user_id']])
end
