...
get '/like_list' do
  ...
  @likes = connection.exec('select * from "like" inner join post on "like".post_id = post.id where "like".user_id = $1 order by post.id desc',[session[:id]]).first
  erb :like_list
end 
...
get '/show_your_plofile/:id' do
  ...
  if params['id'] == session[:user_id]
    ...
    puts params['id'] #OK
    @your_plofile = connection.exec('select * from users where id = $1',[params['id']])
    @your_posts = connection.exec('select * from post where user_id = $1 order by id desc',[params['id']]).first
...
get '/follow/:user_id' do
  check_login
  connection.exec('insert into follow(following_id) values($1)',[session[:id]])
end
