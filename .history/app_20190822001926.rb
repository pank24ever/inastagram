...
get '/timeline' do
  ...
  @posts.each do |res|
    temp = connection.exec('select * from "like" where user_id = $1 and post_id = $2', [session[:user_id], res['id']]).first

    # 配列とハッシュの違い
    # 配列, Array ： それぞれの要素の重さ・価値・データ型がほぼ同じ・似てる。
    # arr = [1, 2, 3, 4]
    # arr = [true, false, false, true]
    
    #ハッシュ, Hash： 全部で一つという意味合い
    # hash = {'HN' => hep, 'read' => へぷ, "sex" => 男, "job" => 大学生}

    # res[:id]   駄目っぽい
    # res['id']  これだった。
    # ※ res['id']とres[:id]は違うキーと要素のセットだよ

    # 存在しないキーを指定した時のサンプル例
    # hash = [{'あ' => true}, {'あ' => true}, {'あ' => true}]
    # p hash    # [{"あ"=>true}, {"あ"=>true}, {"あ"=>true}]
    # hash.each do |res|
    #   res['new'] = false
    # end
    # p hash    # [{"あ"=>true, "new"=>false}, {"あ"=>true, "new"=>false}, {"あ"=>true, "new"=>false}]

    if temp
      like_infos[res['id']] = true
    else
      like_infos[res['id']] = false
    end
  end
  p @like_infos.to_a
  erb :timeline
