...
get '/timeline' do
  check_login
  @title = "TIME LINE"
  @posts = connection.exec('select post.id,post.title,post.contents,post.img,post. user_id,users.id,users.plofile_img from users inner join post on users.id = post.user_id order by post.id desc')
  erb :timeline
end
