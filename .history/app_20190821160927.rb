...
get '/timeline' do
  check_login
  @title = "TIME LINE"
  @posts = connection.exec('select * from post inner join users on post.user_id = users.id order by post.id desc').first
  erb :timeline
end
...
get '/like/:post_id' do
  check_login
  post_id = connection.exec('select * from "like" where user_id = $1 and post_id = $2',[session[:user_id],params[:post_id]])
  if post_id #LIKEテーブルにいいねしている情報がなければ、いいね押すことができる
