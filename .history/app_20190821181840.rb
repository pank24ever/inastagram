...
get '/timeline' do
  check_login
  @title = "TIME LINE"
  @posts = connection.exec('select post.id,post.title,post.contents,post.img,post. user_id,users.id,users.plofile_img from post inner join users on post.user_id = users.id order by post.id desc')
  erb :timeline
end
