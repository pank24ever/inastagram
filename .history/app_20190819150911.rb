...
get '/follow/:user_id' do
  check_login
  connection.exec('insert into follow(following_id,followed_id) values($1,$1)',[session[:id],params['id']])
...
get '/unfollow/:user_id' do
  check_login
  connection.exec('delete from follow where user_id = $1 and post_id = $2',[session[:user_id],params[:post_id]])
end
